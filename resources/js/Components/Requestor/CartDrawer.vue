...
import { Trash2, ScrollText, ShoppingCart, AlertCircle } from 'lucide-vue-next';
...
const openConfirmDialog = ({ title, message, confirmText = 'Confirm', cancelText = 'Cancel', destructive = false, type, item = null }) => {
    confirmDialog.value = { show: true, title, message, confirmText, cancelText, destructive, type, item };
};
...
const handleConfirmAction = () => {
    if (confirmDialog.value.type === 'remove' && confirmDialog.value.item) {
        router.delete(route('requestor.cart.destroy', confirmDialog.value.item.id), {
            preserveScroll: true,
            onSuccess: () => showToast('Item removed from your request list.'),
            onError: () => showToast('Unable to remove the item. Please try again.', 'error'),
        });
    }
                ...
                <div v-if="show"
                    ...
                    <div class="flex-1 overflow-y-auto p-5 space-y-4">
                        <div v-if="!cart?.items?.length"
                            ...
                            <h2 class="text-lg font-semibold text-gray-700 mb-1">Your request list is empty</h2>
                            <p class="text-sm text-gray-500 mb-4">Start adding items to keep track of your requests.</p>
                        </div>
                            ...
                            <div v-for="item in cart.items" :key="item.id"
                                ...
                                <div class="flex-1 space-y-1">
                                    <p class="text-xs text-gray-400">
                                        {{ item.item_code }}
                                        <span class="text-xs text-gray-500 ml-2">({{ item.item_unit }})</span>
                                    </p>
                                    <p class="font-medium text-gray-800 leading-snug">{{ item.item_description }}</p>
                                </div>
                    ...
                    <div class="p-5 border-t bg-white">
                        <button @click="promptSubmitRequest" :disabled="checkoutForm.processing || !cart?.items?.length"
                            ...
                            {{ checkoutForm.processing ? 'Submitting...' : 'Submit Request' }}
                        </button>
                    </div>
                </div>
            </transition>
