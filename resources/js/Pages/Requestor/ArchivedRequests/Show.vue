...
const confirmEdit = () => {
    ...
    router.patch(route('requestor.requests.reopen', props.supplyRequest.id), {}, {
        onSuccess: () => {
            ...
            // Redirect is handled server-side to catalog page
        },
        onError: (errors) => {
            isEditModalOpen.value = false;
            showToast(errors.error ?? 'Could not reopen request.', 'error');
        },
        onFinish: () => { isProcessing.value = false; },
    });
};
    ...
    <AppLayout>
            ...
            <div class="xl:col-span-2 flex flex-col gap-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <!-- M3 DR Number -->
                        <div class="flex items-start gap-3">
                            <div class="p-2 rounded-lg mt-0.5" :class="supplyRequest.m3_dr_number
                                ? 'bg-emerald-50 text-emerald-700'
                                : 'bg-gray-50 text-gray-400'">
                                <Hash class="w-4 h-4" />
                            </div>
        ...
        <Modal :show="isCancelModalOpen" @close="isCancelModalOpen = false" maxWidth="md">
            <div class="p-6">
                ...
                <div class="flex justify-center gap-4 mt-8">
                    <button @click="isCancelModalOpen = false" class="btn-secondary" :disabled="isProcessing">
                        Keep Request
                    </button>
        ...
        <Modal :show="isEditModalOpen" @close="isEditModalOpen = false" maxWidth="md">
            <div class="p-6">
                ...
                <div class="mt-4 text-center">
                    <h3 class="text-lg font-black text-gray-900">Edit This Request?</h3>
                    <p class="mt-2 text-sm text-gray-500">
                        This will reopen
                        <span class="font-bold text-gray-800">{{ supplyRequest.transaction_id }}</span>
                        and return it to your draft.
                        <br><br>
                        You'll be taken back to the Supplies Catalog where you can
                        add, remove, or adjust items. Re-submit when you're done.
                    </p>
                </div>
                <div class="flex justify-center gap-4 mt-8">
                    <button @click="isEditModalOpen = false" class="btn-secondary" :disabled="isProcessing">
                        Go Back
                    </button>
                    <button @click="confirmEdit"
                        class="inline-flex items-center gap-2 px-4 py-2 text-sm font-bold text-white bg-amber-500 rounded-lg border border-amber-600 hover:bg-amber-600 transition-colors disabled:opacity-60"
